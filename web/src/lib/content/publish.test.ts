import assert from "node:assert/strict";
import { test } from "node:test";
import { createMemoryDocumentDb, createWritableStore, seedDocumentDb } from "./d1.ts";
import { publishEvent, publishProject, publishResource, type Actor } from "./publish.ts";

const project = {
  slug: "alpha-bot",
  name: "Alpha",
  tagline: "Voice agents for campus",
  description: "A club Project.",
  category: "AI / ML",
  tags: ["nlp"],
  contributors: [{ name: "Grace Hopper", slug: "grace-hopper", kind: "member" as const }],
};

const event = {
  slug: "intro-talk",
  title: "Intro to Transformers",
  type: "Talk" as const,
  date: "2024-03-12",
  description: "A club talk.",
  links: [],
  contributors: [{ name: "Grace Hopper", slug: "grace-hopper", kind: "member" as const }],
};

const resource = {
  slug: "intro-notes",
  title: "Intro notes",
  description: "Write-up from the talk.",
  type: "Blog" as const,
  addedOn: "2024-03-12",
  body: "Attention is all you need, for a workshop.",
};

function leadership(): Actor {
  return {
    session: { kind: "kerberos", kerberos: "oc1", exp: 0 },
    level: "oc",
    memberSlug: "ada-lovelace",
  };
}

function coordinatorListed(): Actor {
  return {
    session: { kind: "kerberos", kerberos: "coord1", exp: 0 },
    level: "coordinator",
    memberSlug: "grace-hopper",
  };
}

function blogger(): Actor {
  return {
    session: { kind: "kerberos", kerberos: "blog1", exp: 0 },
    level: "blogger",
    memberSlug: "blogger",
  };
}

test("Leadership save then getProject shows the new tagline", async () => {
  const db = createMemoryDocumentDb();
  await seedDocumentDb(db, { projects: [project] });
  const store = createWritableStore(db);

  const result = await publishProject(store, leadership(), {
    ...project,
    tagline: "Campus voice, rewritten",
  });
  assert.equal(result.ok, true);
  assert.equal((await store.getProject("alpha-bot"))?.tagline, "Campus voice, rewritten");
  const log = await store.listChangeLog("projects", "alpha-bot");
  assert.equal(log.length, 1);
  assert.equal(log[0]?.actor, "ada-lovelace");
});

test("a Coordinator not Listed is refused", async () => {
  const db = createMemoryDocumentDb();
  await seedDocumentDb(db, { projects: [project] });
  const store = createWritableStore(db);
  const coordinator: Actor = {
    session: { kind: "kerberos", kerberos: "coord1", exp: 0 },
    level: "coordinator",
    memberSlug: "not-on-the-project",
  };

  const result = await publishProject(store, coordinator, {
    ...project,
    tagline: "Should not stick",
  });
  assert.equal(result.ok, false);
  assert.equal((await store.getProject("alpha-bot"))?.tagline, "Voice agents for campus");
});

test("a Listed Coordinator can direct-publish an Event", async () => {
  const db = createMemoryDocumentDb();
  await seedDocumentDb(db, { events: [event] });
  const store = createWritableStore(db);

  const result = await publishEvent(store, coordinatorListed(), {
    ...event,
    venue: "LH 108, rewritten",
  });
  assert.equal(result.ok, true);
  assert.equal((await store.getEvent("intro-talk"))?.venue, "LH 108, rewritten");
});

test("a Blogger can publish a Resource", async () => {
  const db = createMemoryDocumentDb();
  await seedDocumentDb(db, { resources: [resource] });
  const store = createWritableStore(db);

  const result = await publishResource(store, blogger(), {
    ...resource,
    title: "Intro notes, edited",
  });
  assert.equal(result.ok, true);
  assert.equal((await store.getResource("intro-notes"))?.title, "Intro notes, edited");
});

test("a Blogger cannot publish a Project", async () => {
  const db = createMemoryDocumentDb();
  await seedDocumentDb(db, { projects: [project] });
  const store = createWritableStore(db);

  const result = await publishProject(store, blogger(), {
    ...project,
    tagline: "Should not stick",
  });
  assert.equal(result.ok, false);
  assert.equal((await store.getProject("alpha-bot"))?.tagline, "Voice agents for campus");
});
